...
/**
 * Open Dependabot (security) alert counts per repo — the "npm audit"-style
 * signal for vulnerable dependencies. Counts only RUNTIME dependencies
 * (devDependency alerts are excluded via `dependencyScopes`). Batched across
 * repos, but resilient: `vulnerabilityAlerts` needs admin access + Dependabot
 * enabled, so on a permission error we fall back to per-repo and record `null`
 * (unknown) for the ones we can't read, instead of failing the run. Returns Map
 * of "owner/name" -> count | null.
 */
export async function fetchVulnerabilityCounts(repoNames, { duration, batchSize = 40 } = {}) {
  const counts = new Map();
  const block = (fullName, alias) => {
    const [owner, name] = fullName.split("/");
    return `${alias}: repository(owner:${JSON.stringify(owner)},name:${JSON.stringify(name)}){ vulnerabilityAlerts(states: OPEN, dependencyScopes: [RUNTIME]){ totalCount } }`;
  };

  for (const group of chunk(repoNames, batchSize)) {
    try {
      const data = await graphql(
        `query{ ${group.map((n, i) => block(n, `r${i}`)).join("\n")} }`,
        {},
        duration,
      );
      group.forEach((n, i) => {
        counts.set(n, data[`r${i}`]?.vulnerabilityAlerts?.totalCount ?? null);
      });
    } catch {
      // A repo in the batch is unreadable — retry each on its own so one
      // inaccessible repo doesn't blank out the rest.
      for (const n of group) {
        try {
          ...
          counts.set(n, data.r?.vulnerabilityAlerts?.totalCount ?? null);
        } catch {
          counts.set(n, null); // no access / Dependabot off — unknown
        }
      }
    }
  }
