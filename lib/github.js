...
// Pages of 100 hit GitHub's ~10s GraphQL timeout, which returns an empty body and triggers a stale cache fallback.
const REPOS_QUERY = `
...
/**
 * Pull the `packages:` globs out of a pnpm-workspace.yaml. pnpm monorepos (e.g.
 * sveltejs/svelte) declare workspaces here instead of in package.json, so
 * without this their members would be invisible. Only this one key is needed, so
 * we scan lines rather than take on a YAML dependency. Negated globs are skipped.
 */
function pnpmWorkspaceGlobs(yaml) {
  if (!yaml) return [];
  const globs = [];
  ...
  for (const line of yaml.split(/\r?\n/)) {
    ...
    if (item) {
      const glob = item[1].replace(/^['"]|['"]$/g, "").replace(/\s+#.*$/, "");
      if (glob && !glob.startsWith("!")) globs.push(glob);
      continue;
    }
    if (!line.trim() || /^\s*#/.test(line)) continue; // blank/comment inside block
...
// Repos pinned to a profile (max 6 per user/org). Pins can point at repos the
// profile doesn't own — e.g. a user pinning an org's repo — so callers collect
// pins from every configured login into one set.
const PINNED_QUERY = `
query($login: String!) {
  repositoryOwner(login: $login) {
    ... on User {
...
async function pinnedForOwner(login, duration) {
  try {
    const data = await graphql(PINNED_QUERY, { login }, duration);
    return (data.repositoryOwner?.pinnedItems?.nodes ?? [])
      .map((n) => n?.nameWithOwner)
      .filter(Boolean);
  } catch {
    return []; // pins are cosmetic — never fail a run over them
  }
}
...
function normalizeRepo(repo) {
  ...
  return {
    ...
    isPinned: false, // filled in by discoverRepos
 ...
 */
async function expandWorkspaces(repo, globs, duration) {
  ...
  if (repo.isFork || repo.isPrivate || repo.isDisabled) return [];
  ...
  for (const [filePath, text] of texts) {
    ...
    members.push({
      ...
      isPrivatePackage: Boolean(pkg.private),
      // `isTemplate` describes the REPO — a starter you generate new repos from.
      // A package that merely lives inside such a repo isn't itself a template,
      // so members never inherit the flag (unlike repo-level signals such as
      // stars, issues, and `isPinned`, which genuinely apply to everything in it).
      isTemplate: false,
...
export async function discoverRepos(users = [], orgs = [], { duration } = {}) {
  ...
  // Mark repos pinned to any of the configured profiles.
  const pinned = new Set(
    (await Promise.all(logins.map((l) => pinnedForOwner(l, duration)))).flat(),
  );
  for (const repo of base) repo.isPinned = pinned.has(repo.nameWithOwner);

  // Workspaces come from package.json's `workspaces` field, or — for pnpm
  // monorepos — from pnpm-workspace.yaml. Probe that file for every repo that
  // didn't declare workspaces in package.json, batched into a few requests.
  const globsByRepo = new Map(
    base.map((r) => [r.nameWithOwner, workspaceGlobs(r.packageJson)]),
  );
  const needsProbe = base.filter(
    (r) =>
      !globsByRepo.get(r.nameWithOwner).length &&
      !r.isFork &&
      !r.isPrivate &&
      !r.isDisabled,
  );
  if (needsProbe.length) {
    const yamls = await fetchFilesAcrossRepos(
      needsProbe.map((r) => ({
        key: r.nameWithOwner,
    ...
    for (const [repoName, yaml] of yamls) {
      globsByRepo.set(repoName, pnpmWorkspaceGlobs(yaml));
    }
  }
  ...
  const members = (
    await Promise.all(
      base.map((repo) =>
        expandWorkspaces(repo, globsByRepo.get(repo.nameWithOwner), duration),
      ),
    )
  ).flat();
  return [...base, ...members];
