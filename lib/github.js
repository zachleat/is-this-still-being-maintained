...
/**
 * Fetch one file from each of many repos in batched GraphQL requests (aliased
 * `repository` blocks), instead of a request per repo. `requests` is a list of
 * { key, repo: "owner/name", path }. Returns a Map of key -> file text (missing
 * files are simply absent).
 */
export async function fetchFilesAcrossRepos(requests, { duration, batchSize = 30 } = {}) {
  const texts = new Map();
  for (const group of chunk(requests, batchSize)) {
    const parts = group.map((req, i) => {
      const [owner, name] = req.repo.split("/");
      return `r${i}: repository(owner:${JSON.stringify(owner)},name:${JSON.stringify(name)}){ object(expression:${JSON.stringify(`HEAD:${req.path}`)}){ ... on Blob { text } } }`;
    });
