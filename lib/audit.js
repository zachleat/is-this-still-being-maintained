 ...
 *
 * Returns a Map of package name -> { vulnerabilities, transitiveDependencies },
 * both `null` when the tree couldn't be resolved (unknown rather than "zero").
 */
export async function auditPackages(packages, { duration, concurrency = 8 } = {}) {
  ...
  const unknown = { vulnerabilities: null, transitiveDependencies: null };
  let vulnByDep = new Map();
  try {
    vulnByDep = await osvVulnerabilityCounts([...unique.values()], duration);
  } catch {
    return new Map(packages.map((p) => [p.name, unknown])); // OSV down
  }

  // 3. Total advisories across each package's tree, plus the tree's size.
  const results = new Map();
  for (const pkg of packages) {
    const deps = trees.get(pkg.name);
    if (deps == null) {
      results.set(pkg.name, unknown);
      continue;
    }
    let total = 0;
    ...
    results.set(pkg.name, {
      ...
      transitiveDependencies: deps.length,
    });
  }
  return results;
}
