 ...
 * that closed before the current month can never change, so they never expire
 * from the cache (`closedDuration: "*"`) — only the open window refetches
 * normally, at the caller's `duration`.
 */
export async function fetchDownloadHistory(
  entries,
  { duration, closedDuration = "*", historyYears = 1, now = new Date() } = {},
) {
  ...
  // Month-ALIGNED horizon: start at the first of the month, not this day-of-month
  // N years ago. Starting mid-month would make the leading bucket a partial month
  // that silently undercounts — the same trap the trailing month poses.
  const horizon = addMonths(startOfMonth(now), -12 * historyYears);
