...
export function scoreProject(project, scoring, now = Date.now()) {
  ...
  // Demand: log-scaled, weighted backlog of open issues + PRs. Zero open work ->
  // demand == floor.
  ...
  // Security: open (runtime) vulnerability alerts as a SEPARATE term that
  // bypasses the staleness gate — an unpatched vuln is urgent even on an
  // actively-pushed repo. `securityWeight` caps how much it can drive the score.
  const securitySub = logScale(
    project.openVulnerabilities || 0,
    vulnerabilitiesSaturation,
  );
  const security = securitySub * securityWeight;

  // Attention needed = the worse of ongoing neglect or an open security problem.
  // An archived repo is an explicit "this is no longer maintained" declaration,
  // so nothing is being asked of it: it always scores 0, however big its backlog.
  // The neglect/security sub-scores are still reported for transparency.
  const attention = project.isArchived ? 0 : Math.max(neglect, security);
  ...
  const score = 100 * attention * importance;
